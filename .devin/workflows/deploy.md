...
```
ssh brian@100.115.11.8 "cd ~/.openclaw/workspace/projects/game-show && git pull && npm install && npm run build && cd server && npm install && npm run build && ~/.npm-global/bin/pm2 restart game-show-server --update-env"
```
