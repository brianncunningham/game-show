...
|---|---|---|
| **Song** | Reuses NTT's existing Spotify Connect integration (`src/features/spotify/useSpotify.ts`) as-is. Admin fields are a plain "Spotify Track ID" text box — checked NTT's `ContentManager.tsx`, it's just a pasted ID, not a live search picker (the old implementation-plan doc's "Spotify search picker" idea was never actually built for NTT, so none was built here either) — plus an optional "Start (ms)" number field (`mediaStartMs`). One start position, not NTT's separate clip-start/chorus-start pair — a host picks whichever single spot identifies the song for this question. | Host's Spotify account/device plays audio through the synced speaker — the browser tab never plays it. `TToTOHostComponent` calls `spotify.play(mediaRef, mediaStartMs ?? 0)` directly (same pattern as `NTTHostComponent`) whenever the current question is a song, on both Reveal and Replay. Show screen renders a "🎵 NOW PLAYING" badge, no actual audio. |
