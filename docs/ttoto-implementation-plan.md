...
| Date | Item | Decision |
|---|---|---|
| 2026-09-03 | Q3 — False-start penalty | **Overturned.** `ignore` under-punishes: a controller that just holds/mashes its buzzer through the entire wait wins the race on reflexes-that-aren't-reflexes, not reaction time. `lockout` is the real rule going forward — "you can buzz in once the question AND the answers are both showing; any earlier buzz locks you out for that question" — enforced by opening the hardware window (unarmed/WAITING) the moment a question starts, not just an instant before it arms (see routes.ts's `openWaitingWindow`, added alongside this decision). `earlyBuzzPenalty` stays a per-game `/gameadmin` toggle (ignore/lockout already existed there), not hardcoded — the timing fix just makes lockout actually mean what it says for the whole wait, not only the last few milliseconds before arming. |
| 2026-09-03 | Q7 — Flavor → letter-style pairing | **Closed, no change.** Checked the shipped code: letter style was never actually flavor-paired — `pickLetterStyle()` is pure random rotation (just never repeats the immediately-preceding round's style). The recommendation above was never implemented, and doesn't need to be: `media_id` ("ID Please") answers turned out to be general text (landmarks, movie lines, song titles), not the "years/short codes" this recommendation assumed, so there's no content-shape reason to force segmented for it. |
| 2026-09-03 | Video media type, >3 answer choices, Supabase content pipeline, real 14-segment font | **Confirmed out of scope**, no change from §8's original "explicitly out of scope" call. |
| 2026-09-03 | Pattern B (mixed-mechanic variety show, concept §6) | **Confirmed not pursued** — platform-level idea, no product need for it; navigating between standalone game modes via `/gameadmin` between sessions is sufficient. |
| 2026-09-03 | Long-answer readability (not one of the original numbered Qs — found via live testing with `media_id`/"ID Please" song+artist strings, but affects any flavor) | Per-answer plain-text fallback (`PlainTextRow` in `TToTOShowComponent.tsx`) whenever `fitTextToWidth()` needs to wrap to a 2nd line — dot-matrix/segmented go illegible and split-flap's mid-phrase wrap looks like a glitch, well before shrinking alone stops working. Decided per-answer, not per-flavor or per-round: a question's other, shorter answers keep the mechanical display. |
