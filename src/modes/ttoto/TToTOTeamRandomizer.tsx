// Straight port of Survey Says's SSTeamRandomizer.tsx (same bubble-physics chaos ->
// settle -> done sequence, same structure/timings), adapted for TToTO's palette/branding
// and its hardware-player-only buzzer model (SS's 'hardware-team' per-team badge branch
// doesn't apply here — TToTO only ever shows per-player controller badges, in
// 'hardware-player' mode).
import { Box, Stack, Typography } from '@mui/material';
import { useEffect, useRef, useState } from 'react';
import type { BuzzerMode, ControllerAssignment, TToTOTeam } from './types';
...
// rgb(...) prefixes (alpha appended per-use below) — reuses the mode's own established
// hues (team blue/magenta + choice cyan/orange) rather than introducing new ones.
const BUBBLE_COLORS = ['rgba(59,130,246,', 'rgba(236,72,153,', 'rgba(34,211,238,', 'rgba(249,115,22,'];
...
export const TToTOTeamRandomizer = ({ teams, playerPool, controllerAssignments = [], buzzerMode, onDone }: Props) => {
  const isHardwarePlayer = buzzerMode === 'hardware-player';
  ...
  return (
    <Box sx={{
      ...
      background: 'radial-gradient(ellipse 90% 70% at 50% 40%, rgba(10,25,55,0.96) 0%, rgba(3,4,15,1) 65%)',
      ...
      <Box sx={{ position: 'relative', zIndex: 2, textAlign: 'center', pt: '3vh' }}>
        <Typography sx={{
          ...
          color: phase === PHASE_DONE ? TTOTO_COLORS.warning : TTOTO_COLORS.this,
          textShadow: phase === PHASE_DONE
            ? `0 0 22px ${TTOTO_COLORS.warning}e6`
            : `0 0 18px ${TTOTO_COLORS.this}e6`,
          transition: 'color 600ms ease, text-shadow 600ms ease',
        }}>
          {phase === PHASE_CHAOS ? '🎲 Drawing Teams…' : phase === PHASE_SETTLE ? '⚡ Assigning…' : '✅ Teams Set!'}
        </Typography>
        <Typography sx={{ ...fontSx, color: `${TTOTO_COLORS.warning}8c`, fontSize: 'clamp(0.7rem, 1.1vw, 1.3rem)', letterSpacing: '0.3em', textTransform: 'uppercase', mt: 0.5 }}>
            ...
            {settled && (
              <Stack spacing={1.2} alignItems="center" sx={{ width: '100%', px: 2 }}>
                {team.players.map((p, pi) => {
                  const assignment = isHardwarePlayer
                  ...
                  return (
                    <Box key={p} sx={{
                      ...
                      opacity: 0,
                      animation: `ttotoFadePlayer 500ms ease ${pi * 120}ms forwards`,
                      '@keyframes ttotoFadePlayer': {
...
export default TToTOTeamRandomizer;
